...
class EcofiExportColumns(models.AbstractModel):
    ...
    def get_legal_datev_header(self, ecofi):
        ...
        create_date = '{}000'.format(datetime.now().strftime('%Y%m%d%H%M%S'))
        date_from = datetime.strptime(str(ecofi.date_from), '%Y-%m-%d')
        date_to = datetime.strptime(str(ecofi.date_to), '%Y-%m-%d')
