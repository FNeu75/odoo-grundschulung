...
class PartnerAccountConfiguration(models.Model):
    ...
    def update_shared_sequences(self) -> None:
        ...
        for account_type in ['payable', 'receivable']:
            config_sequence_field_name = 'account_{type}_sequence_id'.format(
                type=account_type,
            )
    ...
    def _create_accounts(self, partner, account_types: list) -> dict:
        ...
        for account_type in account_types:
            field = 'property_account_{account_type}_id'.format(
            ...
            for config in self:
                if not self._is_default_account(
                    ...
                    continue
                account = config._create_account(
                    partner,
                    account_type,
                )
                setattr(
    ...
    def _is_default_account(self, partner, company, account_type: str) -> bool:
        ...
        default_field = 'property_account_{account_type}_is_default'.format(
            account_type=account_type,
        )
    ...
    def _create_account(self, partner, account_type: str, code: str = None):
        ...
        return self.env['account.account'].create({
            ...
            'user_type_id':
                self.env.ref(
                    'account.data_account_type_{account_type}'.format(
                        account_type=account_type,
                    ),
                ).id,
        })
    ...
    def _account_code(self, account_type: str) -> str:
        field = 'account_{account_type}_sequence_id'.format(
            account_type=account_type,
        )
        return getattr(self, field).next_by_id()
