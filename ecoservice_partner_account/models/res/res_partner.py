...
class ResPartner(models.Model):
    ...
    def _compute_account_is_default(self):
        ...
        for partner in self:
            default_payable = partner._fields[payable].default(partner)
            default_receivable = partner._fields[receivable].default(partner)

            partner.property_account_payable_is_default = (
                default_payable == partner[payable]
            )
            partner.property_account_receivable_is_default = (
                default_receivable == partner[receivable]
            )
    # endregion
